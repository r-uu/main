...
public interface TaskGroup<T extends Task<?, ?>>
{
	...
	/**
	 * Removes a task from the internal set of tasks ({@link #tasks()}). A task always belongs to a task group. The
	 * internal set of tasks for a task group has to be updated by a call to this method.
