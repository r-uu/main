package de.ruu.app.jeeeraaah.frontend.ui.fx.task.gantt;

import de.ruu.app.jeeeraaah.frontend.ui.fx.model.TaskFXBean;
import javafx.beans.property.ReadOnlyStringWrapper;
import javafx.beans.property.StringProperty;
import javafx.beans.value.ObservableValue;
import javafx.scene.control.TreeTableColumn.CellDataFeatures;
import javafx.util.Callback;
import lombok.NonNull;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

import java.time.LocalDate;
import java.util.Optional;

@Slf4j
@RequiredArgsConstructor
public class TaskTreeTableCellValueFactory
		implements Callback<CellDataFeatures<TaskFXBean, String>, ObservableValue<String>>
{
	private final @NonNull LocalDate date;

	@Override public StringProperty call(CellDataFeatures<TaskFXBean, String> cdf)
	{
		return readOnlyStringWrapperFor(cdf.getValue().getValue());
//		ReferenceCycleTracking context = new ReferenceCycleTracking();
//		return readOnlyStringWrapperFor(toBean(taskFXBean, context));
	}

	private ReadOnlyStringWrapper readOnlyStringWrapperFor(TaskFXBean task)
	{
		return readOnlyStringWrapperFor(task.start(), task.end());
	}

	private ReadOnlyStringWrapper readOnlyStringWrapperFor(Optional<LocalDate> start, Optional<LocalDate> end)
	{
		if (start.isPresent())
		{
			// known startDate
			LocalDate startDate = start.get().minusDays(1);
			if (end.isPresent())
			{
				// startDate and endDate are known so full in period check is possible
				LocalDate endDate = end.get().plusDays(1);
				if (date.isAfter(startDate) && date.isBefore(endDate))
						// date is inside period
						return new ReadOnlyStringWrapper("x");
				// date is outside period
				return new ReadOnlyStringWrapper("");
			}
			// known startDate and unknown end of period means that as long as date is after startDate it is unknown if date is
			// inside period
			if (date.isAfter(startDate))
					// end is unknown
					return new ReadOnlyStringWrapper("<?");
			// date is outside period
			return new ReadOnlyStringWrapper("");
		}
		else
		{
			// unknown begin
			if (end.isPresent())
			{
				// unknown begin and known endDate means that as long as date is before endDate it is unknown if date is inside period
				LocalDate endDate = end.get().plusDays(1);
				if (date.isBefore(endDate))
						// begin is unknown
						return new ReadOnlyStringWrapper("?<");
				// date is outside period
				return new ReadOnlyStringWrapper("");
			}
			return new ReadOnlyStringWrapper("<?<");
//			return new ReadOnlyStringWrapper("");
		}
	}

//	private ReadOnlyStringWrapper readOnlyStringWrapperFor(TaskBean taskBean)
//	{
////		return new ReadOnlyStringWrapper("bla");
//		if (taskBean.start().isPresent())
//		{
//			// known begin
//			LocalDate begin = taskBean.start().get().minusDays(1);
//			if (taskBean.end().isPresent())
//			{
//				// begin and end are known so full in period check is possible
//				LocalDate end = taskBean.end().get().plusDays(1);
//				if (date.isAfter(begin) && date.isBefore(end))
//					// date is inside period
//					return new ReadOnlyStringWrapper("x");
//				// date is outside period
//				return new ReadOnlyStringWrapper("");
//			}
//			// known begin and unknown end of period means that as long as date is after begin it is unknown if date is
//			// inside period
//			if (date.isAfter(begin))
//				// end is unknown
//				return new ReadOnlyStringWrapper("<?");
//			// date is outside period
//			return new ReadOnlyStringWrapper("");
//		}
//		else
//		{
//			// unknown begin
//			if (taskBean.end().isPresent())
//			{
//				// unknown begin and known end means that as long as date is before end it is unknown if date is inside period
//				LocalDate end = taskBean.end().get().plusDays(1);
//				if (date.isBefore(end))
//					// begin is unknown
//					return new ReadOnlyStringWrapper("?<");
//				// date is outside period
//				return new ReadOnlyStringWrapper("");
//			}
//			if (taskBean.superTask().isPresent())
//				// period is unknown
//				return new ReadOnlyStringWrapper("<?<");
//			return new ReadOnlyStringWrapper("");
//		}
//	}
}
